...
function appendStreamChoiceContent(choice, contentParts, reasoningParts) {
  const deltaContent = choice?.delta?.content;
  const messageContent = choice?.message?.content;
  const textContent = choice?.text;
  // 推理模型的推理增量：delta.reasoning / delta.reasoning_content（OpenAI 兼容网关常见两种命名）
  const deltaReasoning = choice?.delta?.reasoning ?? choice?.delta?.reasoning_content;
  const messageReasoning = choice?.message?.reasoning ?? choice?.message?.reasoning_content;
  if (reasoningParts) {
    if (typeof deltaReasoning === 'string') reasoningParts.push(deltaReasoning);
    else if (typeof messageReasoning === 'string') reasoningParts.push(messageReasoning);
  }
...
async function readOpenAIChatStream(response) {
  const state = { usage: null, contentParts: [], reasoningParts: [] };
  ...
  await readSseJsonStream(response, {
    ...
    onPayload(payload) {
      ...
      choices.forEach((choice) => appendStreamChoiceContent(choice, state.contentParts, state.reasoningParts));
    },
  });

  // 兼容推理模型：正常 content 为空时回退用 reasoning 流内容（其中往往含 ```json 代码块，
  // 由上层 parseJsonContent 提取）。
  const rawContent = state.contentParts.join('');
  const content = rawContent.trim() ? rawContent : state.reasoningParts.join('');
  return {
    content,
    usage: state.usage,
    responseData: {
...
async function requestTextAiNormal(app, config, requestBody, options = {}) {
  ...
  // 兼容推理模型（如 sensenova-*-flash）：开启 response_format=json_object 时，
  // 部分推理模型把全部输出塞进 message.reasoning，而 message.content 为空，
  // 导致上层 parseJsonContent('') 失败并无限重试。这里在 content 为空时回退取 reasoning。
