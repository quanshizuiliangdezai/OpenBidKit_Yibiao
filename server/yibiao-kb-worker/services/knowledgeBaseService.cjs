...
let dialog = { showOpenDialog: async () => ({ canceled: true, filePaths: [] }) };
try { dialog = require('electron').dialog; } catch { /* 服务器 Node 环境无 electron，分析链路不使用 dialog */ }
...
function createKnowledgeBaseService({ app, aiService, configStore, knowledgeBaseStore, kbTeamService }) {
  ...
  // 记录团队库分析回写（saveAnalysis）的结果，供服务器 Worker 判断任务是否真正成功。
  // 客户端场景「本机已完成、回写失败不阻塞用户」是合理的，但 Worker 的唯一目的就是回写，
  // 因此 Worker 需要读这个 Map 才能区分「真成功」与「本地成功但回写失败」的假成功。
  // 结构：documentId(String) -> { ok: boolean, message?: string }
  const teamShareResult = new Map();

  // 统一的团队库分析回写：从本地 store 完整重建 payload 并 POST 到服务器 kb_analysis。
  ...
  async function shareTeamAnalysisFromStore(documentId, libraryType, webContents) {
    ...
    try {
      ...
      const finalItems = knowledgeBaseStore.readItems(documentId);
      ...
      const sharedPayload = {
        ...
        markdown,
        parser_label: (doc && doc.parser_label) || null,
        blocks,
  ...
  async function matchDocument(documentId, webContents, options = {}) {
    ...
    try {
      ...
      if (!force && saveStep?.status === 'success' && savedItems.length) {
        ...
        // 缓存命中也必须回写团队库，否则 Worker 重跑（step 全缓存）永远不同步到服务器。
      ...
      // 与 save:reuse 分支共用同一个从 store 重建 payload 的函数，保证两条路径回写行为一致。
      await shareTeamAnalysisFromStore(documentId, libraryType, webContents);
    } catch (error) {
  ...
  return {
    ...
    async analyzeExternalFile(documentId, filePath, fileName, folderId, webContents, libraryType) {
      // 服务器环境下 documentId/folderId 均可能是 number（来自 sqlite 数字主键），
      // 但下游 path.join / sqlite 主键均要求 string，这里统一归一化为字符串，避免
      // "path argument must be of type string" 崩溃。客户端本地是 UUID 字符串，转换无副作用。
      documentId = String(documentId);
      const normalizedFolderId = (folderId === undefined || folderId === null || folderId === '') ? null : String(folderId);
    ...
    // ---- 服务器 Worker 专用：与 analyzeExternalFile 相同，但 await 整条分析管线 ----
    // 客户端版 analyzeExternalFile 立即返回 savedDocument，靠 webContents 事件异步更新进度，
    // 适合 UI 不阻塞。但服务器 Worker 需要「等分析真正完成 + 回写 kb_analysis」后才能标记任务结束，
    // 因此这里 await prepareDocument。建记录逻辑与 analyzeExternalFile 完全一致（保持零算法改动）。
    async analyzeExternalFileAwait(documentId, filePath, fileName, folderId, webContents, libraryType) {
      documentId = String(documentId);
      const normalizedFolderId = (folderId === undefined || folderId === null || folderId === '') ? null : String(folderId);
      if (libraryType) {
        documentLibraryType.set(String(documentId), libraryType);
      ...
      else knowledgeBaseStore.createDocument(documentFields);
      // await：等整条管线（转 MD → 切块 → 抽取 → team 库 saveAnalysis 回写）跑完。
