...
function createKbAuthService({ app }) {
  ...
  // 服务器 Worker 场景：进程长驻、单次分析可能跑数分钟，启动时的 token 会过期。
  // 注册重登凭据后，apiFetch 在遇到 401 时自动重新登录并重试一次，对业务调用透明。
  // 客户端场景不注册凭据（走 onUnauthorized 弹门禁），因此不受影响。
  let reloginCredentials = null;
  function setReloginCredentials(cred) {
    reloginCredentials = cred && cred.username ? { ...cred } : null;
  }
  ...
  async function apiFetch(apiPath, { method = 'GET', body = null, headers = {}, timeoutMs = null } = {}) {
    const isFormData = body !== null
      && (body instanceof FormData || (typeof FormData !== 'undefined' && body?.constructor?.name === 'FormData'));

    // 单次发送。返回 { ok, status, data }。抽成内部函数便于 401 后重登重试。
    async function doSend() {
      ...
      let res;
      try {
        // 大 payload（如团队库分析回写，动辄数 MB）在单线程 Python http.server 上落库耗时可能
        // 远超默认 10s，调用方可传入更长 timeoutMs 覆盖，避免 AbortController 中途 abort。
    ...
    // Worker 长跑 token 过期：401 时若已注册重登凭据，自动重登并重试一次（FormData 流已消费，不重试）。
    if (result.status === 401 && reloginCredentials && !isFormData) {
      try {
        await login({
          username: reloginCredentials.username,
          password: reloginCredentials.password,
          serverUrl: reloginCredentials.serverUrl,
        });
        console.warn('[apiFetch] 会话过期，已自动重登并重试:', apiPath);
        result = await doSend();
      } catch (reloginErr) {
        console.error('[apiFetch] 自动重登失败:', reloginErr?.message || reloginErr);
      }
    }

    // 令牌失效（重试后仍 401）：通知上层，便于渲染进程重新弹出门禁
