...
def delete_employee(user_id):
    """管理员删除账号：先清会话，再删其文档文件、文档行、文件夹（级联子文件夹与文档），最后删账号。禁止删除管理员。"""
    try:
        user_id = int(user_id)
    except (TypeError, ValueError):
        return False, '无效的用户 ID'
    with _lock:
        ...
        try:
            ...
            conn.execute("DELETE FROM sessions WHERE employee_id=?", (user_id,))
            docs = conn.execute(
                "SELECT id,file_path FROM knowledge_documents WHERE owner_id=?", (user_id,)).fetchall()
            for d in docs:
                _remove_file(d['file_path'])
            conn.execute("DELETE FROM knowledge_documents WHERE owner_id=?", (user_id,))
            conn.execute("DELETE FROM knowledge_folders WHERE owner_id=?", (user_id,))
