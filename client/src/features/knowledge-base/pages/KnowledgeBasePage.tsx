...
function KnowledgeBasePage() {
  ...
  // ============ 模型配置面板（个人库本地分析 / 团队库分析 / 问答 统一设置）============
  const [showModelConfig, setShowModelConfig] = useState(false);
  const [mcBaseUrl, setMcBaseUrl] = useState('');
  const [mcApiKey, setMcApiKey] = useState('');
  const [mcModel, setMcModel] = useState('');
  const [mcModels, setMcModels] = useState<string[]>([]);
  const [mcLoadingModels, setMcLoadingModels] = useState(false);
  const [mcTesting, setMcTesting] = useState(false);
  const [mcSavingLocal, setMcSavingLocal] = useState(false);
  const [mcSavingGlobal, setMcSavingGlobal] = useState(false);
  const [mcGlobalEmbedding, setMcGlobalEmbedding] = useState('');
  const [mcFullConfig, setMcFullConfig] = useState<Record<string, unknown> | null>(null);
  ...
  const openModelConfig = async () => {
    ...
    try {
      ...
      if (cfg) {
        ...
        setMcBaseUrl((profile.base_url as string) || '');
        setMcApiKey((profile.api_key as string) || '');
        setMcModel((profile.model_name as string) || '');
      }
    } catch {
      /* 忽略读取失败 */
    }
    if (isAdmin) {
      try {
        ...
        if (res?.success && res.data) {
          setMcGlobalEmbedding(res.data.embedding_model || '');
          if (res.data.base_url) setMcBaseUrl(res.data.base_url);
          if (res.data.analysis_model) setMcModel(res.data.analysis_model);
        }
      } catch {
        /* 忽略读取失败 */
      }
  ...
  const fetchMcModels = async () => {
    if (!mcBaseUrl.trim()) { showToast('请先填写 API Base URL', 'info'); return; }
    if (!mcApiKey.trim()) { showToast('请先填写 API Key', 'info'); return; }
    setMcLoadingModels(true);
    try {
      const probe = { ...(mcFullConfig || {}), base_url: mcBaseUrl.trim(), api_key: mcApiKey.trim(), model_name: mcModel.trim() };
      const result = await window.yibiao?.config.listModels(probe as never);
      const models = result?.models || [];
      setMcModels(models);
      if (result?.success && models.length) {
        showToast(`获取到 ${models.length} 个模型`, 'success');
        if (!models.includes(mcModel)) setMcModel(models[0]);
      } else {
      ...
      setMcLoadingModels(false);
    }
  };

  const testMcConnection = async () => {
    if (!mcBaseUrl.trim() || !mcApiKey.trim()) { showToast('请先填写 API Base URL 与 API Key', 'info'); return; }
    setMcTesting(true);
    try {
      const probe = { ...(mcFullConfig || {}), base_url: mcBaseUrl.trim(), api_key: mcApiKey.trim(), model_name: mcModel.trim() };
      const result = await window.yibiao?.config.listModels(probe as never);
  ...
  const saveMcLocal = async () => {
    if (!mcBaseUrl.trim() || !mcApiKey.trim() || !mcModel.trim()) {
    ...
    setMcSavingLocal(true);
    try {
      const base = mcFullConfig;
      const provider = 'custom';
      const profiles = (base.text_model_profiles as Record<string, Record<string, unknown>>) || {};
      const prev = profiles[provider] || {};
      const newProfile = {
        ...(base.text_model_provider === provider ? prev : {}),
        api_key: mcApiKey.trim(),
        base_url: mcBaseUrl.trim(),
        model_name: mcModel.trim(),
      ...
      setMcSavingLocal(false);
    }
  };

  const applyMcGlobal = async () => {
    if (!mcBaseUrl.trim() || !mcModel.trim()) {
      showToast('Base URL 与模型名称不能为空', 'error');
      return;
    }
    setMcSavingGlobal(true);
    try {
      const res = await window.yibiao?.config.saveGlobal({
        base_url: mcBaseUrl.trim(),
        api_key: mcApiKey.trim() ? mcApiKey.trim() : '__UNCHANGED__',
        analysis_model: mcModel.trim(),
        qa_model: mcModel.trim(),
        embedding_model: mcGlobalEmbedding || null,
      });
    ...
    <>
      <div className="page-stack knowledge-page">
        ...
        <div className="knowledge-toolbar-actions">
          ...
          <button type="button" className="secondary-action" onClick={() => void openModelConfig()}>模型配置</button>
        </div>
      </section>
      ...
      <Dialog.Root open={showModelConfig} onOpenChange={(open) => !open && setShowModelConfig(false)}>
        <Dialog.Portal>
          <Dialog.Overlay className="knowledge-sync-modal" />
          <Dialog.Content className="knowledge-sync-dialog-card" onCloseAutoFocus={(event) => event.preventDefault()}>
            <button type="button" className="knowledge-dialog-close" onClick={() => setShowModelConfig(false)} aria-label="关闭">×</button>
            <div className="knowledge-sync-head">
              <h3 className="knowledge-sync-title">模型配置</h3>
              <p className="knowledge-sync-desc">设置后个人库本地分析、团队库服务器分析、问答都将使用该模型。可分别保存到本机与服务器（管理员）。</p>
            </div>

            <div className="knowledge-sync-form">
              <label className="knowledge-sync-field">
                <span>API Base URL</span>
                <input
                  value={mcBaseUrl}
                  onChange={(event) => setMcBaseUrl(event.target.value)}
              ...
              <label className="knowledge-sync-field">
                ...
                <input
                  type="password"
                  value={mcApiKey}
                  onChange={(event) => setMcApiKey(event.target.value)}
                  placeholder={isAdmin ? '留空 = 沿用服务器现有密钥' : '请输入 API Key'}
                />
              </label>
              <label className="knowledge-sync-field">
                <span>模型名称</span>
                <div className="knowledge-sync-model-row">
                  <input
                    list="kb-model-list"
                    value={mcModel}
                    onChange={(event) => setMcModel(event.target.value)}
                    placeholder="agens2.5"
                  />
                  <datalist id="kb-model-list">
                    {mcModels.map((m) => (
                      <option key={m} value={m} />
                    ))}
                  </datalist>
                  <button type="button" className="secondary-action" onClick={() => void fetchMcModels()} disabled={mcLoadingModels}>
                    {mcLoadingModels ? '获取中…' : '获取模型'}
                  </button>
                </div>
              </label>
            </div>
            ...
            <p className="knowledge-sync-hint">
              「获取模型」按上方 Base URL + Key 拉取可用模型；「测试连接」验证可达性与鉴权；「保存到本机」用于个人库本地分析，「应用到服务器」用于团队库分析与问答（需管理员）。
            </p>

            <div className="knowledge-sync-actions">
              <button type="button" className="secondary-action" onClick={() => void testMcConnection()} disabled={mcTesting}>
                {mcTesting ? '测试中…' : '测试连接'}
              </button>
              <button type="button" className="primary-action" onClick={() => void saveMcLocal()} disabled={mcSavingLocal}>
                {mcSavingLocal ? '保存中…' : '保存到本机'}
              </button>
              {isAdmin && (
                <button type="button" className="sync-action" onClick={() => void applyMcGlobal()} disabled={mcSavingGlobal}>
                  {mcSavingGlobal ? '应用中…' : '应用到服务器'}
                </button>
              )}
            </div>
          </Dialog.Content>
