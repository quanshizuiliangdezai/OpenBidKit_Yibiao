...
function createPiUserQuestionTool({ Type, requestUserQuestion }) {
  return {
    ...
    promptGuidelines: [
      ...
      '选项名称使用“按推荐方案生成”“保持当前目录”等可直接判断结果的简短操作文案，不得描述程序处理步骤或数据结构。',
      '普通选项设置 custom=false；只有需要用户选中后输入具体要求时才设置 custom=true。custom=true 的选项最多只能有一个，通常放在最后，且必须使用“调整目录安排”等明确业务名称，不得使用含义模糊的“其他”。',
    ],
