...
5.1 所有一级目录标题必须互不相同，不得出现完全重复的标题。注意：标题是否使用“实施方案”“技术方案”等词取决于招标文件本身，不要为规避重复而强行改写——若评分表中“实施方案（各技术方向）”是聚合型评分项（细目共享同一评分项分值），则应只生成 1 个一级目录“实施方案”、技术方向归入 detail_points（见第 11、12 条）；只有真正独立的评分项才单独成一级目录且标题互不相同。
...
11. 聚合型评分项处理（重要）：如果评分表中出现“一个评分项标题覆盖多个技术方向、各技术方向只是该评分项下的评分细目（共享同一评分项、合并计分）”的情况，必须按以下规则处理：
  - 典型形态：评分项标题形如“实施方案（各技术方向）”“技术方案（各技术方向）”，或其下技术方向（网络组网、安全防范系统、计算机及存储等）列在“细分列 / 细目行”中，而非各自拥有独立的评分项标题单元格与独立分值行；
  - 这种情况只允许生成 **1 个一级目录**，标题使用评分项的核心词（如“实施方案”），去掉“（各技术方向）”之类的聚合说明；
  - 各技术方向必须作为该一级目录的 detail_points，绝对不得各自成为一级目录，也不得在标题中写成“实施方案（网络组网）”这种多一级目录形式。
12. 独立评分项判定：只有当某个技术方向在评分表中是“独立评分项行”（拥有自己的评分项标题单元格与独立的满分分值）时，才允许它单独成为一级目录；否则一律归入所属评分项的 detail_points。
...
4.1 所有一级目录标题必须互不相同，不得出现完全重复的标题。若响应文件要求中“实施方案”是一个聚合章节（其下分项共享该章节），则只生成 1 个一级目录“实施方案”、分项归入 detail_points；不要为规避重复而强行改写标题。只有真正独立的文件目录才单独成一级目录。
...
const OUTLINE_WIZARD_STEP_PROGRESS = Object.freeze({
  extract: OUTLINE_PROGRESS.mainComplete,
...
function getOutlineWizardOrderedSteps({ isExpansionWorkflow, outlineExpansionMode }) {
  if (isExpansionWorkflow) {
    if (outlineExpansionMode === 'original-only') {
      return ['extract'];
    }
